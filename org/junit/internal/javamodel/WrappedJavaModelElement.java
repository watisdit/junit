...
public abstract class WrappedJavaModelElement extends JavaModelElement {
	public WrappedJavaModelElement(JavaTestInterpreter interpreter) {
		super(interpreter);
	}
	...
	void runAfters(Object test, RunNotifier runNotifier) {
		// TODO: train wreck
		for (Method after : getJavaClass().getMethods(getAfterAnnotation())) {
			try {
				after.invoke(test);
			} catch (Throwable e) {
	...
	void runBefores(Object test, RunNotifier runNotifier) throws FailedBefore {
		try {
	...
	public void runWithBeforeAndAfter(Runnable protectThis, Object test,
